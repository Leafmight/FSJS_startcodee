...
import {POSITION_COLLECTION_NAME,POST_COLLECTION_NAME} from "../src/config/collectionNames"
...
export default class GameFacade {
  ...
  static async nearbyPlayers(userName: string, password: string, longitude: number, latitude: number, distance: number) {
    ...
    try {
      ...
      if(!loggedIn) throw new Error(); //DELETE ME????
    ...
    try {
      ...
      /*It's important you know what to do her. Remember a document for this user does
        ...
        short time */
      const found = await positionCollection.findOneAndUpdate({userName},{$set:{userName,name: user.name, lastUpdated: date, location: point}},{upsert:true, returnOriginal: false})
