...
export default class UserFacade {
    ...
    static async deleteUser(userName: string): Promise<string> {
        const status = userCollection.findOneAndDelete({userName})
        if((await status).ok){
    ...
    static async getAllUsers(): Promise<Array<any>> {
        const all = await userCollection.find({},{projection:{_id:0, password:0, name:1, userName:1}})
        return all.toArray();
        //throw new Error("Not Implemented")
    }

    static async getUser(userName: string,proj?:object): Promise<any> {
        const user = await userCollection.findOne({userName},{projection:{proj}})
        if(!user){
